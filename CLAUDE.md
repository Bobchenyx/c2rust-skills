...
- **FFI boundary at BLOCKING modules only**: Modules with BLOCKING patterns (inline asm, computed goto, setjmp) are marked `ffi_boundary = true` in the manifest and kept as C with hand-written FFI bindings. The pipeline does NOT support mid-pipeline mixed builds — convert processes all non-BLOCKING modules in a single pass.
